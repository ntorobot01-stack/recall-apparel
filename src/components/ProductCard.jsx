  ...
  return (
    <article className="card group">
      ...
      >
        ...
        <div className="absolute top-4 right-4 bg-black/85 text-white px-2.5 py-1 text-[10px] font-mono tracking-[0.18em] uppercase">
          {product.geo_tag}
        </div>
      </button>
      ...
      <div className="pt-5 px-1">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[11px] font-mono uppercase tracking-[0.18em] text-[var(--muted)]">{product.brand}</span>
          <span className="text-[11px] font-mono uppercase tracking-[0.16em] text-[var(--muted)]">{product.store}</span>
        </div>

        <h3 className="text-[15px] font-medium leading-snug mb-3">{product.name}</h3>

        <div className="flex items-baseline gap-3 mb-5">
          <span className="text-[18px] font-semibold">{fmt(product.price, product.currency)}</span>
          <span className="text-[13px] line-through text-[var(--muted)]">{fmt(product.originalPrice, product.currency)}</span>
        </div>
        ...
        <div className="mt-3 flex items-center justify-center gap-1.5 text-[10.5px] text-[var(--muted)]">
          ...
          <span>Pago 100% seguro en la tienda oficial</span>
        </div>
      </div>
    </article>
  )
